...
# Type alias compatible with vLLM 0.18.1 (Tool class not yet in abstract_tool_parser)
Tool = dict


def safe_literal_eval(value: str) -> Any:
    """Safely evaluate a string literal using ast.literal_eval."""
    try:
...
class HYV3ToolParser(ToolParser):
    ...
    def __init__(self, tokenizer: TokenizerLike, tools: list[Tool] | None = None):
        ...
        self.tool_calls_start_token: str = "<tool_calls:opensource>"
        self.tool_calls_end_token: str = "</tool_calls:opensource>"

        self.tool_call_start_token: str = "<tool_call:opensource>"
        self.tool_call_end_token: str = "</tool_call:opensource>"

        self.tool_sep_token: str = "<tool_sep:opensource>"

        self.arg_key_start_token: str = "<arg_key:opensource>"
        self.arg_key_end_token: str = "</arg_key:opensource>"

        self.arg_value_start_token: str = "<arg_value:opensource>"
        self.arg_value_end_token: str = "</arg_value:opensource>"
        ...
        if (
            ...
            vocab = self.vocab
            matching = {k: v for k, v in vocab.items() if 'tool_call' in k.lower() and 'opensource' in k}
            raise RuntimeError(
                "HYV3 Tool parser could not locate tool call "
                "start/end tokens in the tokenizer! "
                f"Looked for: start={self.tool_calls_start_token!r} "
                f"(id={self.tool_calls_start_token_id}), "
                f"end={self.tool_calls_end_token!r} "
                f"(id={self.tool_calls_end_token_id}). "
                f"Matching tokens in vocab: {matching}. "
                f"Vocab size: {len(vocab)}, "
                f"tokenizer type: {type(self.model_tokenizer).__name__}"
            )
