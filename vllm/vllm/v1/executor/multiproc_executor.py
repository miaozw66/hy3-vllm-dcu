    ...
    def collective_rpc(  # type: ignore[override]
        ...
        if self.rpc_broadcast_mq is None:
            ...
            if isinstance(method, str):
                if method in ("compile_or_warm_up_model",):
                    return [0.0 for _ in range(self.local_world_size)]
                if method in ("determine_available_memory",):
                    return [0 for _ in range(self.local_world_size)]
                if method in ("initialize_from_config",):
                    return None
            return [{} for _ in range(self.local_world_size)]
        if self.is_failed:
