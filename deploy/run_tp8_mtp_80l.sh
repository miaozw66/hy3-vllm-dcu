#!/bin/bash
# Single-node TP=8 full 80-layer HY3 launch script WITH MTP speculative decoding.
# Draft: checkpoint layer 80 (num_nextn_predict_layers=1), spec depth = 1.
#
# Usage:
#   bash deploy/run_tp8_mtp_80l.sh               # eager mode (-O0, default)
#   MODE=graph bash deploy/run_tp8_mtp_80l.sh    # CUDA graph mode (-O1)
#   DEBUG_MODE=1 bash deploy/run_tp8_mtp_80l.sh  # full INFO logs (debug)
#
# Notes:
#   - MTP runtime was validated on the 2-node PP=2 deployment with -O0 +
#     --no-async-scheduling; the same scheduling flags are kept here.
#   - With PP=1 the drafter is built on every TP rank (all ranks are last
#     PP rank) and the MTP layer weights are TP-sharded like the main model.
set -e
...
# env.sh 已为 LL+Tree；此处 Simple/Ring 覆盖是 0903 trace/0829-0831 的口径。
# 去掉覆盖让 LL/Tree 生效并无 profiler TPOT A/B 复测。注意：任何跨 run 数值
# 对比（含 A/B）必须保持被对比双方协议一致，否则上面的 INT8 漂移陷阱仍适用。
...
# HY3 MTP draft: checkpoint has 1 nextn layer (num_nextn_predict_layers=1),
# speculative depth = 1.
...
echo "=== Full 80-Layer HY3 Single-Node TP=8 + MTP Launch (mode=$MODE, debug=$DEBUG) ==="
echo "Started at: $(date)"
echo "Model: $MODEL_PATH"
echo "Spec config: $SPEC_CONFIG"
...
NODE0_LOG="$LOG_DIR/vllm_tp8_mtp_80l_${MODE}_$(date +%m%d_%H%M).log"
echo "  Log: $NODE0_LOG"
