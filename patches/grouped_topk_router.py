...
def grouped_topk(
    ...
    if e_score_correction_bias is not None:
        # MetaInfer-compatible: apply bias BEFORE sigmoid, use biased scores
        # for both expert selection and routing weights.
        biased_logits = gating_output + e_score_correction_bias.unsqueeze(0)
    else:
        biased_logits = gating_output

    if scoring_func == "softmax":
        scores = torch.softmax(biased_logits, dim=-1)
